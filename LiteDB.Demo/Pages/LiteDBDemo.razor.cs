...
namespace LiteDB.Demo.Pages
{
    ...
    public partial class LiteDBDemo:IAsyncDisposable
    {
        [Inject]
        ...
        private async Task<ILiteDatabase> GetDb(bool refersh = false, bool open = true, bool useCaceh = false)
        {
            if (this.db == null || refersh)
            {
                ...
                {
                    cfg.UseCache = useCaceh;
                    //cfg.UseReadCaches = readCache;
                });
            }
        ...
        {
            var db = await this.GetDb(useCaceh: false);
        ...
        {
            var db = await this.GetDb(useCaceh:true);
